...
import shutil,hashlib,random,time,zipfile,os,json,subprocess,codecs,re,glob,logging,tarfile,textwrap,traceback,sys
...
class djmarinara:
    def __init__(self,extensions=['zip','xm','it','s3m','mod','mp3','mp4','flac','m4a','aac','flv','3gp', 'ogg', 'ra', 'rm'],
            temppath="/tmp/songs",
            mediapath="/media",
            playlisturl="",
            fonturl="",
    ...
    def convertFile(self,file):
        ...
        convert = subprocess.Popen(["ffmpeg",
                                    ...
                                    "-hide_banner",
                                    "-filter_complex",
                                    r"[0:a]showcqt=sono_h=0:axis=0:s=1920x1080:fps=30:bar_h=1080:cscheme=1|0|1|0|1|0:csp=bt470bg[left]; [left] hflip [left]; [left] drawtext=fontfile=font.ttf:fontsize=24:fontcolor=white:x=20:y=h-mod(max(t-0.0\,0)*(h+th)/50.0\,(h+th)):textfile=" + filedata['textfile'] + " [out]; [out] fade=t=in:st=0:d=5,fade=t=out:st=" + str(fadeouttime) + ":d=5 [out]",
                                    "-map","[out]",
                                    "-map","0:a",
                                    "-c:v","libx264",
    ...
    def makeText(self,data):
        ...
        for l in outlines:
            if len(l)>self.linewidth:
                outstring += "\n".join(textwrap.wrap(l,width=self.linewidth,expand_tabs=False,replace_whitespace=False,drop_whitespace=False,break_long_words=False,break_on_hyphens=False))
            else:
                outstring += l
        print("Wrapped lines:")
        print(outstring)
